...
namespace Framework.Extension
{
    ...
    {
        static T CreateConstructorObject<T>() where T : class
        {
            var type = typeof(T);
            // 获取私有构造函数 - Add 增加Public的构造函数 私有构造函数 在代码裁切的情况下 可能出问题
        ...
        public static T CreateSingleton<T>() where T : class, ISingleton
        {
            ...
            if (monoBehaviourType.IsAssignableFrom(type)) //MONO
            {
                var instance = CreateMonoSingleton<T>();
                instance.OnSingletonInit();
                return instance;
            }
            else
            {
                var instance = CreateConstructorObject<T>(); //OTHER
                instance.OnSingletonInit();
                return instance;
            }
        }
    ...
    /// <summary>
    /// Mono单例 需要自己挂载在场景中
    /// 如果需要动态创建 请使用 SingletonCreator.CreateSingleton<T>
    /// </summary>
