...
namespace Logic.Fight.Common
{
    ...
    public class FightEnemySpawnManager : MonoSingleton<FightEnemySpawnManager>
    {
        ...
        public async UniTask<Enemy> SpawnEnemy(string pResName)
        {
            ...
            var _Enemy = await SpawnEnemy(pResName, iEnemyGroundIndex);
            iEnemyGroundIndex++;
            return _Enemy;
        ...
        public async UniTask<Enemy> SpawnEnemy(string pResName, int spawnPositionIndex)
        {
            Vector3 _Pos = m_EnemyGroundSpawnPoss[spawnPositionIndex].position;
            _Pos.y += RandomHelper.Range(-0.2f, 0.2f);
        ...
        {
            var _Enemy = await SpawnEnemy(pResName, m_EnemyGroundSpawnPoss.Length -1);
            return _Enemy;
        }
