...
namespace Logic.UI.UIUser
{
    public class UISpark : UIPage
    {
        ...
        private void Init()
        {
            InstantiateEngine(EngineManager.Ins.sparkMap.Values.ToList());
            RefreshEngine();
            OnTecPointChanged();
        }
        ...
        private void UpdateEngine()
        {
            ...
            var pQuality = sparkData.Quality;
            var pResID = sparkData.ResID;
            var pName = sparkData.Name;
            var pTecPoint = sparkData.Decompose;
        ...
        private void InstantiateRemoveEngine()
        {
            ...
            removeSelectedGet = 0;
            UpdateRemoveEngine();
            foreach (var gameEngineData in EngineManager.Ins.sparkMap.Values.ToList())
            {
        ...
        private void OnEnginePartsUpdate(int eventId, object data)
        {
            ...
            var sparkList = pList.Where(gameEnginePartData => (ItemType)gameEnginePartData.Type == ItemType.Spark)
                .ToList();
            InstantiateEngine(sparkList);
        }
        ...
        {
            if (EngineManager.Ins.sparkMap.Count >= 1)
            {
