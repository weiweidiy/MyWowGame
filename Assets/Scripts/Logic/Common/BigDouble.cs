...
namespace BreakInfinity
{
    ...
    {
        private static readonly string[] NumSuffix =
        {
            "",
            "A",
            "B",
        ...
        public string ToUIString()
        {
            ...
            var v1 = _e / 3;
            var v2 = _e % 3;
            var v3 = _v * Mathf.Pow(10, v2);
            
            //不满1000做取整处理
            if (_e < 3)
            {
                var tmp = Mathf.Pow(10, _e);
                v3 =  tmp * Mathf.Floor((float)(_v * tmp)) / tmp;
            }
            
            string formattedNum = v3.ToString("###.");
            
            if (v2 == 0)
                formattedNum = v3.ToString("#.##");
            else if (v2 == 1)
                formattedNum = v3.ToString("##.#");
            else if (v2 == 2)
                formattedNum = v3.ToString("###.");
            if (formattedNum.Length <= 0 )
                formattedNum = "0";
            var s = $"{formattedNum}{NumSuffix[v1]}";
            
            return s;
        }
        
        public string ToUIStringFloat()
        {
            var _v = mantissa;
            var _e = exponent;
        ...
        public static BigDouble AbsLess(BigDouble value, int e)
        {
            //Pow( 10 ,e )以内的取整
            if (value.Exponent < e)
                if (value.exponent == 0 && value.mantissa < 1)
                    value = 1;
                value =FromMantissaExponentNoNormalize(Math.Floor(value.Mantissa), value.Exponent);
            return value;
        }
